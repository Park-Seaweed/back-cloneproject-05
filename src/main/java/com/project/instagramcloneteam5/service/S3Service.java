...
public class S3Service {
    ...
    public List<String> upload(List<MultipartFile> multipartFile) {
        ...
        for (MultipartFile file : multipartFile) {
            ...
            try(InputStream inputStream = file.getInputStream()) {
                s3Client.putObject(new PutObjectRequest(bucket+"/board/image", fileName, inputStream, objectMetadata)
    ...
    // 이미지 삭제
    public void delete(List<Image> imgList) {
        try {
            imgList.stream().forEach(i -> s3Client.deleteObject(new DeleteObjectRequest(bucket+"/board/image", i.getImgUrl().split("amazonaws.com/")[1])));
